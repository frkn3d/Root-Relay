...
function drawArcherTower(t){
  ...
  capGrad.addColorStop(0,'#c3f0b8'); capGrad.addColorStop(0.45,'#8fc482'); capGrad.addColorStop(1,'#446f3f');
  ctx.beginPath(); ctx.ellipse(0,0,24,15,0,Math.PI,0);
  ctx.fillStyle=capGrad; ctx.fill(); ctx.lineWidth=2.5; ctx.strokeStyle='#22391d'; ctx.stroke();
  ...
  [[-11,-6,3.2],[4,-10,2.6],[12,-3,2.2],[-2,-4,1.8]].forEach(([dx,dy,r])=>{
...
function drawMageTower(t){
  ...
  shards.forEach(([dx,dy,s],i)=>{
    ...
    g.addColorStop(0,'#eafffa'); g.addColorStop(0.5,'#7fe3c4'); g.addColorStop(1,'#2c8067');
    ctx.fillStyle=g; ctx.fill(); ctx.lineWidth=1.8; ctx.strokeStyle='#0d241c'; ctx.stroke();
    ctx.beginPath(); ctx.moveTo(-s*0.15,-s*0.6); ctx.lineTo(-s*0.1,s*0.3);
    ctx.strokeStyle='rgba(255,255,255,0.6)'; ctx.lineWidth=1; ctx.stroke();
    ctx.restore();
  });
  // çekirdek parlaması (katmanlı)
  [16,10,5].forEach((r,i)=>{
    ctx.beginPath(); ctx.arc(0,-32,r+((t.pulse||0)*3),0,Math.PI*2);
    ctx.fillStyle = i===2 ? '#ffffff' : `rgba(180,255,235,${0.25-i*0.08})`;
    ctx.fill();
  });
  ctx.shadowColor='#4fc3a1'; ctx.shadowBlur=18;
  ctx.beginPath(); ctx.arc(0,-32,4,0,Math.PI*2); ctx.fillStyle='#eafffa'; ctx.fill();
  ctx.shadowBlur=0;
  ctx.restore();

  // yörüngede dönen ışık zerreleri
  for(let i=0;i<3;i++){
    const ang = t0*1.6 + i*(Math.PI*2/3);
...
function drawIceTower(t){
  ...
  shards.forEach(([dx,dy,s],i)=>{
    ...
    g.addColorStop(0,'#ffffff'); g.addColorStop(0.55,`hsl(${196+hueShift},70%,75%)`); g.addColorStop(1,`hsl(${210+hueShift},55%,55%)`);
    ctx.fillStyle=g; ctx.fill(); ctx.lineWidth=1.6; ctx.strokeStyle='#1c3540'; ctx.stroke();
  ...
  for(let i=0;i<3;i++){
    ...
    const sx = x-14+i*14, sy = y-46+cyc*46;
...
function drawMortarTower(t){
  ...
  grad.addColorStop(0,'#eeb27a'); grad.addColorStop(0.6,'#c9793f'); grad.addColorStop(1,'#8a4a20');
  ...
  [[-13,4],[13,4],[0,13],[-9,11],[9,11]].forEach(([dx,dy])=>{
    ctx.beginPath(); ctx.arc(x+dx,y+dy,1.4,0,Math.PI*2);
    ctx.fillStyle='#5c3a1e'; ctx.fill();
  });
  ...
  const barrelGrad = ctx.createLinearGradient(-6,-26,6,0);
  barrelGrad.addColorStop(0,'#5c5c56'); barrelGrad.addColorStop(0.5,'#3a3a34'); barrelGrad.addColorStop(1,'#4a4a44');
  ctx.fillStyle=barrelGrad; ctx.strokeStyle='#1a1a17'; ctx.lineWidth=2.5;
